...
func (cmd *ATCCommand) constructMembers(
	...
	authHandler, err := skymarshal.NewServer(&skymarshal.Config{
		...
		DexServerURL:       cmd.ExternalURL.String(),
		SkyServerURL:       cmd.ExternalURL.String(),
		TLSConfig:          tlsConfig,
		Expiration:         cmd.Auth.AuthFlags.Expiration,
		GithubClientID:     cmd.Auth.AuthFlags.GithubFlags.ClientID,
		GithubClientSecret: cmd.Auth.AuthFlags.GithubFlags.ClientSecret,
		LocalUsers:         cmd.Auth.AuthFlags.LocalUserFlags,
	})
	if err != nil {
		return nil, err
	}
